...
<!--Modifica CSI: eliminazione SchemaLocation per problemi del validatore interno GN3 tanto il validatore RNDT non lo considera-->
 <xsl:template match="@xsi:schemaLocation"/>  
   ...
   <!--<xsl:template match="//gmd:parentIdentifier">
      ...
      <!--Modica CSI: Spostamento ParentIdentifier del metadato su gmd:source-->
	  ...
	  <xsl:copy>
       ...
       <xsl:value-of select="normalize-space(/gmd:MD_Metadata/gmd:parentIdentifier)"/>
   </xsl:attribute>
   </gmd:source>
   </xsl:if>
    </xsl:copy>
  ...
  <!--Eliminazione ParentiIdentifier dopo spostament valore in gmd:lineage/gmd:LI_Lineage/gmd:source-->
<xsl:template match="gmd:MD_Metadata/gmd:parentIdentifier"/>
   ...
   <!--Modica CSI: Rimozione suffisso resource dall'ID del dato-->
	<xsl:template match="gmd:CI_Citation/gmd:identifier/gmd:RS_Identifier/gmd:code/gco:CharacterString">
	<xsl:copy>
	<xsl:choose>
	  <xsl:when test="contains(.,'resource')" >
	   <xsl:value-of select="concat(substring-before(.,'_resource'),'')"/>
	  </xsl:when>
	  <xsl:otherwise>
		<xsl:apply-templates select="@*|node()"/>
	  </xsl:otherwise>
	...
	<!--Modica CSI: Rimozione suffisso resource dall'ID della serie relativa al dato-->
	<xsl:template match="gmd:series/gmd:CI_Series/gmd:issueIdentification/gco:CharacterString">
	<xsl:copy>
	<xsl:choose>
	  <xsl:when test="contains(.,'resource')" >
	   <xsl:value-of select="concat(substring-before(.,'_resource'),'')"/>
   ...
   <!--Modica CSI: Eliminazione Sezione dove messi metadati per vecchio scarico con GAIMS-->
...
<!--MODIFICA CSI: Add EU new keyword where there's RNDT keyword -->
  <xsl:template match="//gmd:descriptiveKeywords [contains (.,'RNDT')]">
   <xsl:copy-of select="."/>
   ...
   <!--MODIFICA CSI: Resource constraints  sostituisce modificando il pezzo originale-->
<xsl:template match="//gmd:resourceConstraints[gmd:MD_Constraints/gmd:useLimitation]">
            <gmd:resourceConstraints>
         <gmd:MD_LegalConstraints>
            <gmd:accessConstraints>
      ...
      <gmd:resourceConstraints>
        ...
        <xsl:choose>
              ...
              <xsl:when test="contains(translate(gmd:MD_Constraints/gmd:useLimitation/gco:CharacterString, $uppercase, $lowercase),'condizioni sconosciute') or contains (translate(gmd:MD_Constraints/gmd:useLimitation/gco:CharacterString, $uppercase, $lowercase),'condizioni non note')">
               <gmx:Anchor xlink:href="http://inspire.ec.europa.eu/metadata-codelist/ConditionsApplyingToAccessAndUse/conditionsUnknown">Condizioni sconosciute</gmx:Anchor>
              </xsl:when>
              <xsl:otherwise>  
               <gco:CharacterString>
                  <xsl:text>https://creativecommons.org/licenses/by/4.0/deed.it</xsl:text>
               </gco:CharacterString>
         </xsl:otherwise>
            </xsl:choose>  
            </gmd:otherConstraints>
  ...
  <!-- MODIFICA CSI: Remove vertical extent if it's fake!-->
    <xsl:template match="gmd:verticalElement [descendant::gmd:verticalCRS/@xlink:href[. = 'http://www.rndt.gov.it/ReferenceSystemCode#999']]">
    </xsl:template> 
   
   <!-- Modifica CSI - Distribution Format. Sospesa la trasformazione perchè il template del plugin non prevede la gestione del formato con l'Anchor-->
   ...
   <!--MODIFICA CSI: Passaggio all'https per le URL dei geoservizi in tutti i metadati dei servizi nel connectPoint ed eliminazione protocol e name--> 
 <xsl:template match="//srv:SV_OperationMetadata/srv:connectPoint/gmd:CI_OnlineResource">
 <gmd:CI_OnlineResource>
        <gmd:linkage>
			  <gmd:URL>
				<xsl:choose>
	            <xsl:when test="contains(gmd:linkage/gmd:URL,'niger3.csi.it')">
				 <xsl:value-of select="concat('https://opengis.csi.it',substring-after(gmd:linkage/gmd:URL,'http://niger3.csi.it'))"/>
				</xsl:when>
				<xsl:when test="contains(gmd:linkage/gmd:URL,'http://opengis.csi.it')">
				 <xsl:value-of select="concat('https',substring-after(gmd:linkage/gmd:URL,'http'))"/>
				</xsl:when>
				<xsl:when test="contains(gmd:protocol/gco:CharacterString,'WFS') and not(contains(gmd:linkage/gmd:URL,'geovector3.territorio.csi.it')) or contains(gmd:protocol/gco:CharacterString,'WMS') and not(contains(gmd:linkage/gmd:URL,'osgis2.csi.it'))">
				<xsl:value-of select="concat('https',substring-after(gmd:linkage/gmd:URL,'http'))"/>
				</xsl:when>
				<xsl:otherwise>
     ...
     <!--MODIFICA CSI: OperatesOn aggiornamento URL metadati con URL nuova di test--> 
 <xsl:template match="//srv:operatesOn">
 <xsl:copy>
 <xsl:apply-templates select="@*|node()"/>
 <xsl:choose>
    <xsl:when test="contains(./@xlink:href,'http://www.geoportale.piemonte.it/geonetworkrp')">
   <xsl:attribute name="xlink:href"> 
	<xsl:value-of select="concat('https://www.geoportale.piemonte.it/geonetwork',substring-after(./@xlink:href,'http://www.geoportale.piemonte.it/geonetworkrp'))"/>
	</xsl:attribute> 
	</xsl:when>
   <xsl:otherwise>
	<xsl:apply-templates select="@*|node()"/>
   ...
   <xsl:template match="//gmd:MD_Distribution/gmd:transferOptions/gmd:MD_DigitalTransferOptions/gmd:onLine/gmd:CI_OnlineResource">
      <gmd:CI_OnlineResource>
		 ...
		 <xsl:variable name="protocol" select="translate(gmd:protocol/*/text(), $lowercase, $uppercase)" />
            ...
            <!-- MODIFICA CSI: Aggiunta condizioni alla URL per: sostituzione dominio niger3.csi.it con opengis.csi.it e 
			passaggio all'https per tutti i geoservizi-->
			<gmd:linkage>
			  <gmd:URL>
				<xsl:choose>
	            <xsl:when test="contains(gmd:linkage/gmd:URL,'niger3.csi.it')">
				...
				<xsl:when test="contains(gmd:protocol/gco:CharacterString,'WFS') and not(contains(gmd:linkage/gmd:URL,'geovector3.territorio.csi.it')) or contains(gmd:protocol/gco:CharacterString,'WMS') and not(contains(gmd:linkage/gmd:URL,'osgis2.csi.it')) or contains(gmd:linkage/gmd:URL,'.xml')">
				<xsl:value-of select="concat('https',substring-after(gmd:linkage/gmd:URL,'http'))"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="gmd:linkage/gmd:URL"/>
         ...
         <xsl:if test="$hierarchyLevel_loc != 'service'">
           <gmd:linkage>
			<!-- MODIFICA CSI: Aggiunta condizioni alla URL per: sostituzione dominio niger3.csi.it con opengis.csi.it e 
			passaggio all'https per tutti i geoservizi e i pacchetti di scarico datigeo-->
               <gmd:URL>
			   <xsl:choose>
	            <xsl:when test="contains(gmd:linkage/gmd:URL,'niger3.csi.it')">
				 <xsl:value-of select="concat('https://opengis.csi.it',substring-after(gmd:linkage/gmd:URL,'http://niger3.csi.it'))"/>
				...
				<xsl:when test="contains(gmd:linkage/gmd:URL,'http://www.datigeo-piem-download.it/')">
					 <xsl:value-of select="concat('https',substring-after(gmd:linkage/gmd:URL,'http'))"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="gmd:linkage/gmd:URL"/>
   ...
   <!--MODIFICA CSI: Passaggio in https url di scarico dei pacchetti dati su datigeo--> 
 <xsl:template match="//gmd:CI_OnlineResource/gmd:linkage/gmd:URL">
	 <xsl:copy>  
		<xsl:choose>
		<xsl:when test="contains(.,'http://www.datigeo-piem-download.it/')">
...
<!--MODIFICHE CSI - INIZIO-->

<!--Modifica solo per nodi harvestati: aggiunta URL localizzazione thumbnails all'interno del catalogo harvester-->
<xsl:template match="//gmd:graphicOverview/gmd:MD_BrowseGraphic/gmd:fileName/gco:CharacterString">
  <gco:CharacterString>
  <xsl:choose>
    <xsl:when test="contains(.,'.png')">
   	<xsl:value-of select="concat('https://tst-www.geoportale.piemonte.it/geonetwork/srv/api/records/',//gmd:fileIdentifier/gco:CharacterString,'/attachments/',substring-before(.,'png'),'png')"/>
	</xsl:when>
   <xsl:otherwise>
	<xsl:apply-templates select="@*|node()"/>
   </xsl:otherwise>
...
<!--Ripulitura OrganizationName, eliminazione slash per Enti Locali-->
    <xsl:template match="gmd:organisationName/gco:CharacterString[contains(.,'/')]">
	  	  <xsl:copy>
	  	  	       <xsl:value-of select="translate(.,'/','')"/>
		  </xsl:copy>
	</xsl:template>
	 
	<!--Ripulitura statement, eliminazione slash per Enti Locali-->
	<xsl:template match="gmd:lineage/gmd:LI_Lineage/gmd:statement/gco:CharacterString[contains(.,'/')]">
	  	  <xsl:copy>
	  	  	       <xsl:value-of select="translate(.,'/','')"/>
		  </xsl:copy>
	</xsl:template> 
